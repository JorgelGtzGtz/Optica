...
export class ColoresService {
  ...
  private _url = `${AppSettings.API_ENDPOINT}/Color`;
  private _getLista = `${this._url}/Lista`;
  private _getGetColor = `${this._url}/GetColor`;
  ...
  getColor(id: number): Observable<any>  {
    return this._http.get<any>(`${this._getGetColor}/${id}`, { headers: this._userService.header})
      .pipe(
        tap(data => data),
        catchError(this.handleError)
      );
