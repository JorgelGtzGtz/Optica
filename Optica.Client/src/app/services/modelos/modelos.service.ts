...
export class ModelosService {
  ...
  private _url = `${AppSettings.API_ENDPOINT}/Modelos`;
  private _getLista = `${this._url}/Lista`;
  private _getGetModelo = `${this._url}/GetModelo`;
  ...
  getLista(filter: String): Observable<any[]> {
    const params = new HttpParams().set('modelo', filter.toString());
  ...
  getModelo(id: number): Observable<any>  {
    return this._http.get<any>(`${this._getGetModelo}/${id}`, { headers: this._userService.header})
      .pipe(
        tap(data => data),
        catchError(this.handleError)
      );
