...
export class TipoLenteService {
  ...
  private _url = `${AppSettings.API_ENDPOINT}/TiposLente`;
  private _getLista = `${this._url}/Lista`;
  private _getGetTiposLente = `${this._url}/GetTiposLente`;
  ...
  getTipoLente(id: number): Observable<any>  {
    return this._http.get<any>(`${this._getGetTiposLente}/${id}`, { headers: this._userService.header})
      .pipe(
        tap(data => data),
        catchError(this.handleError)
      );
