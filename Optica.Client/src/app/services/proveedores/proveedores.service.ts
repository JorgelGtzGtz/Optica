...
export class ProveedoresService {
  ...
  private _url = `${AppSettings.API_ENDPOINT}/Proveedores`;
  private _getLista = `${this._url}/Lista`;
  private _getGetProveedor = `${this._url}/GetProveedor`;
  ...
  getProveedor(id: number): Observable<any>  {
    return this._http.get<any>(`${this._getGetProveedor}/${id}`, { headers: this._userService.header})
      .pipe(
        tap(data => data),
        catchError(this.handleError)
      );
