...
export class TipoEntradaSalidaService {

  private _url = `${AppSettings.API_ENDPOINT}/TiposEntradaSalida`;
  private _getLista = `${this._url}/Lista`;
  private _getTiposEntradaSalida = `${this._url}/GetTiposEntradaSalida`;
  ...
  getTipoEntradaSalida(id: number): Observable<any>  {
    return this._http.get<any>(`${this._getTiposEntradaSalida}/${id}`, { headers: this._userService.header})
      .pipe(
        tap(data => data),
        catchError(this.handleError)
      );
