...
export class ZonasService {
  ...
  private _url = `${AppSettings.API_ENDPOINT}/Zonas`;
  private _getLista = `${this._url}/Lista`;
  private _getGetZona = `${this._url}/GetZona`;
  ...
  getZona(id: number): Observable<any>  {
    return this._http.get<any>(`${this._getGetZona}/${id}`, { headers: this._userService.header})
      .pipe(
        tap(data => data),
        catchError(this.handleError)
      );
