...
export class SucursalesService {
  ...
  private _url = `${AppSettings.API_ENDPOINT}/Sucursales`;
  private _getLista = `${this._url}/Lista`;
  private _getGetSucursal = `${this._url}/GetSucursal`;
  ...
  getLista(sucursal: String): Observable<any[]> {
    const params = new HttpParams().set('sucursal', sucursal.toString());
  ...
  getSucursal(id: number): Observable<any>  {
    return this._http.get<any>(`${this._getGetSucursal}/${id}`, { headers: this._userService.header})
      .pipe(
        tap(data => data),
        catchError(this.handleError)
      );
