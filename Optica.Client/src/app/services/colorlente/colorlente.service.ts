...
export class ColorLenteService {
  ...
  private _url = `${AppSettings.API_ENDPOINT}/ColorLente`;
  private _getLista = `${this._url}/Lista`;
  private _getGetColorLente = `${this._url}/GetColorLente`;
  ...
  getColorLente(id: number): Observable<any>  {
    return this._http.get<any>(`${this._getGetColorLente}/${id}`, { headers: this._userService.header})
      .pipe(
        tap(data => data),
        catchError(this.handleError)
      );
